...
'value'=>
		...
		array('header'=>'Active Properties','name'=>'cr_ip',
			'type'=>'raw',
            'visible'=>($_GET['id'] =='101')?true:false,
			'value'=>'get_aproperties($data->id)',
		    ),
			
		array('header'=>'InActive Properties','name'=>'crby',
			'type'=>'raw',
            'visible'=>($_GET['id'] =='101')?true:false,
			'value'=>'get_iproperties($data->id)',
		    ),
...
{
	$properties = Pinfo::model()->find(array('select'=>'group_concat(name) as name,status','condition'=>"owner='".$data."'"));
	$pname = $properties->name;
	$status = ( $properties->status == 2 ) ? "(N/A)" : "";
    return $pname.$status;
}

function get_aproperties($data)
{
	$aproperties = Pinfo::model()->find(array(
		'select'=>'group_concat(name) as name',
		'condition'=>"owner='".$data."' AND status = 1"));
		
    return $aproperties->name;
}

function get_iproperties($data)
{
	$iproperties = Pinfo::model()->find(array(
		'select'=>'group_concat(name) as name',
		'condition'=>"owner='".$data."' AND status != 1"
	));
