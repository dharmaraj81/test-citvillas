  ...
  <section id="main-content">
    ...
    <div class="container">
     <?php if( isset( $page->icon_file ) && ( $page->icon_file != '') ) { ?>
      ...
      <div class="fourcol left last">
        <?php $this->renderPartial('//layouts/search-widget-region',array('layout_asset'=>$layout_asset,'SearchWidget'=>$SearchWidget,'RegId'=>$RegId)); ?>
      </div>
     <?php } ?>
    </div>
    <br>
            ...
            <article class="listing list">
              <div class="list-listing-info col span_12">
				...
				<div class="item wishItem">						  
				  <a class="thumb" href="<?php echo $f->weburl; ?>"> <img src="<?php echo Gallery::GetPropThumbnail($f->id); ?>" class="attachment-large wp-post-image" width="306" height="202" alt="<?php echo $f->altTag; ?>"></a> 
                ...
                <header>
                  <h4 class="marT5"><a href="<?php echo $f->weburl; ?>"><?php echo $f->tt_name; ?></a></h4>
                  <p> <?php echo Town::GetName($f->town); ?>, <?php echo Province::GetName($f->province); ?></p>
                </header>
                <div class="price marT0 marB0">
                  
				  <?php if( (prop_min_price_night($f->id)!= 0) || (prop_min_price($f->id)!= 0 ) ) { ?>
				  <?php echo t('From'); ?> 
                  <?php } ?>
				  <?php if(prop_min_price_night($f->id)!= 0 ) { ?>
                   <?php echo Yii::app()->session['currency']; ?> <?php echo prop_min_price_night($f->id); ?> <?php echo t('Per Night'); ?>
                  <?php } ?>
                  <?php if( (prop_min_price_night($f->id)!= 0) && (prop_min_price($f->id)!= 0 ) ) { ?>
				  <?php echo '/'; ?>
                  <?php } ?>
				  <?php if(prop_min_price($f->id)!= 0 ) { ?>
                   <?php echo Yii::app()->session['currency']; ?> <?php echo prop_min_price($f->id); ?> <?php echo t('Per Week'); ?>
                  <?php } ?>
